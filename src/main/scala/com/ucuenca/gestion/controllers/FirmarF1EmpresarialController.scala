...
case class F1EstudianteItem(idPractica: Int, nombre: String) {
  override def toString: String = nombre
}

class FirmarF1EmpresarialController {
  ...
  @FXML var btnSubirF1EmpresaFirmado: Button = _
  @FXML var lblEstadoF1Empresa: Label = _
  ...
  def initialize(): Unit = {
    ...
    cmbEstudiantesF1.setOnAction(_ => {
      ...
      if (seleccion != null) {
        ...
        selectedFile = None
        lblEstadoF1Empresa.setText("Ningún archivo seleccionado (.pdf)")
        btnEnviar.setDisable(true)
  ...
  private def cargarEstudiantesAsignados(): Unit = {
    try {
      val lista = Formulario1DB.listarEstudiantesParaEmpresa(activeTutorCI)
      cmbEstudiantesF1.getItems.clear()
      lista.foreach { case (idPr, nombre) =>
        cmbEstudiantesF1.getItems.add(F1EstudianteItem(idPr, nombre))
      }
      if (lista.isEmpty) {
        showSuccess("No registra expedientes de Formulario 1 pendientes de su firma.")
      }
    } catch {
      case e: Exception =>
  ...
  def handleSubir(event: ActionEvent): Unit = {
    ...
    fileChooser.getExtensionFilters.add(new FileChooser.ExtensionFilter("Archivos PDF (*.pdf)", "*.pdf"))
    val file = fileChooser.showOpenDialog(btnSubirF1EmpresaFirmado.getScene.getWindow)
    if (file != null) {
      selectedFile = Some(file)
      lblEstadoF1Empresa.setText(file.getName)
      lblEstadoF1Empresa.setStyle("-fx-text-fill: #1e293b; -fx-font-style: normal;")
    ...
    (seleccion, selectedFile) match {
      case (sel, Some(file)) =>
        try {
          ...
          Formulario1Logic.tutorEmpresaFirmar(sel.idPractica, bytes, file.getName) match {
            case Right(_) =>
              ...
              showSuccess("¡Firma digital de la empresa cargada y transmitida exitosamente!")
              btnEnviar.setDisable(true)
              vboxAccionesFirma.setDisable(true)
              selectedFile = None
              lblEstadoF1Empresa.setText("Enviado")
              cargarEstudiantesAsignados()
